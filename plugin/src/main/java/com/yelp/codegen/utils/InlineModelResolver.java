...
/**
 * A fork of [{@link io.swagger.codegen.InlineModelResolver} that copies the vendor extensions in
...
public class InlineModelResolver {
    ...
    private Map<String, Model> addedModels = new HashMap<>();
                            ...
                            if (parameter instanceof BodyParameter) {
                                ...
                                if (bp.getSchema() != null) {
                                    ...
                                    if (model instanceof ModelImpl) {
                                        ...
                                        if (obj.getType() == null
                                                || "object".equals(obj.getType())) {
                                            if (obj.getProperties() != null
                                                    && obj.getProperties().size() > 0) {
                                                flattenProperties(obj.getProperties(), pathname);
                                                String modelName =
                                                        resolveModelName(
                                                                obj.getTitle(), bp.getName());
                            ...
                            if (response.getSchema() != null) {
                                ...
                                if (property instanceof ObjectProperty) {
                                    ...
                                    if (op.getProperties() != null
                                        ...
                                        String modelName =
                                                resolveModelName(
                                                        op.getTitle(), "inline_response_" + key);
                                        Model model = modelFromProperty(op, modelName);
                                        String existing = matchGenerated(model);
                                        if (existing != null) {
                                            Property refProperty =
                                                    this.makeRefProperty(existing, property);
                                            refProperty.setRequired(op.getRequired());
                                            response.setSchema(refProperty);
                                        } else {
                                            Property refProperty =
                                                    this.makeRefProperty(modelName, property);
                                            refProperty.setRequired(op.getRequired());
                                            response.setSchema(refProperty);
                                            addGenerated(modelName, model);
                                            swagger.addDefinition(modelName, model);
                                    ...
                                    if (inner instanceof ObjectProperty) {
                                        ...
                                        if (op.getProperties() != null
                                            ...
                                            if (existing != null) {
                                                Property refProperty =
                                                        this.makeRefProperty(existing, op);
                                                refProperty.setRequired(op.getRequired());
                                                ap.setItems(refProperty);
                                            } else {
                                                Property refProperty =
                                                        this.makeRefProperty(modelName, op);
                                                refProperty.setRequired(op.getRequired());
                                                ap.setItems(refProperty);
                                                addGenerated(modelName, innerModel);
                                                swagger.addDefinition(modelName, innerModel);
    ...
    private String matchGenerated(Model model) {
        if (this.skipMatches) {
            return null;
        }
    ...
    private void addGenerated(String name, Model model) {
    ...
    private void flattenProperties(Map<String, Property> properties, String path) {
        if (properties == null) {
            return;
        }
        Map<String, Property> propsToUpdate = new HashMap<>();
        Map<String, Model> modelsToAdd = new HashMap<>();
        for (String key : properties.keySet()) {
            ...
            if (property instanceof ObjectProperty
                ...
                if (existing != null) {
                    RefProperty refProperty = (RefProperty) makeRefProperty(existing, property);
                    refProperty.setRequired(op.getRequired());
                    propsToUpdate.put(key, refProperty);
                } else {
                    RefProperty refProperty = (RefProperty) makeRefProperty(modelName, property);
                    refProperty.setRequired(op.getRequired());
                    propsToUpdate.put(key, refProperty);
                ...
                if (inner instanceof ObjectProperty) {
                    ...
                    if (op.getProperties() != null && op.getProperties().size() > 0) {
                        ...
                        String existing = matchGenerated(innerModel);
                        if (existing != null) {
                            Property refProperty = makeRefProperty(existing, property);
                            refProperty.setRequired(op.getRequired());
                            ap.setItems(refProperty);
                        } else {
                            Property refProperty = makeRefProperty(modelName, property);
                            refProperty.setRequired(op.getRequired());
                            ap.setItems(refProperty);
                            addGenerated(modelName, innerModel);
                            swagger.addDefinition(modelName, innerModel);
    ...
    private Model modelFromProperty(ObjectProperty object, String path) {
        String description = object.getDescription();
        String example = null;
        ...
        copyVendorExtensions(object, model);
     ...
     */
    private Property makeRefProperty(String ref, Property property) {
     ...
     */
    private void copyVendorExtensions(Property source, AbstractProperty target) {
     ...
     * @param target target property
     */
    private void copyVendorExtensions(Property source, ModelImpl target) {
        Map<String, Object> vendorExtensions = source.getVendorExtensions();
        for (String extName : vendorExtensions.keySet()) {
            target.setVendorExtension(extName, vendorExtensions.get(extName));
        }
